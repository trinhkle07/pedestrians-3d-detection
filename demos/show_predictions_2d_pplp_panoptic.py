...
def main():
    """This demo shows RPN proposals and PPLP predictions in 3D
    ...
    # dataset_config.data_split = 'test_mair'
    # dataset_config.data_split = 'test_panoptic'
    dataset_config.data_split = 'val'

    # fig_size = (10, 6.1)
    fig_size_3 = (20, 40)  # The size of final picture as a whole.
    fig_size_2 = (14.2, 25)
    fig_size_2 = (18, 20)

    rpn_score_threshold = 0
    pplp_score_threshold = 0.3
    ...
    global_step = 168865  # occupancy (FMP dataset finetuned)
    # global_step = 109866  # No occupancy (Panoptic)
    # global_step = 119615  # occupancy (Panoptic)
    ...
    # BEV Map format
    bev_format = 'occupancy_maps'  # 'occupancy_maps' or 'density_map'
    ...
    if draw_overlaid:
        overlaid_out_dir = output_dir_base + '/overlaid/{}/{}/{}'.format(
            dataset.data_split, global_step, pplp_score_threshold)
    ...
    for sample_idx in range(dataset.num_samples):
        ...
        if draw_predictions_separate or draw_overlaid:
            ...
            if len(prediction_boxes_3d) > 0:
                ...
                # Apply score mask
                pplp_score_mask = prediction_scores >= pplp_score_threshold
                prediction_boxes_3d = prediction_boxes_3d[pplp_score_mask]
                print('pplp_score_threshold = ', pplp_score_threshold)
                print('There are ', len(prediction_boxes_3d), ' predictions left.')
                prediction_scores = prediction_scores[pplp_score_mask]
                prediction_class_indices = \
                    prediction_class_indices[pplp_score_mask]
        ...
        if bev_format == 'occupancy_maps':
            ...
            if not draw_bev_map:
                bev_img = np.zeros(bev_img.shape, dtype=np.float)
        elif bev_format == 'density_map':
            bev_img = np.array(bev_maps['density_map'], dtype=np.float)
            bev_img = bev_img/np.max(bev_img)*255
            bev_img = np.resize(bev_img, (bev_img.shape[0], bev_img.shape[1]))  # [height, width]
            if not draw_bev_map:
                bev_img = np.zeros((bev_img.shape[0], bev_img.shape[1]), dtype=np.float)
        else:
            raise Exception("bev_format Error!")
        ...
        if draw_proposals_separate or draw_overlaid:
            ...
            if draw_point_cloud:
                ...
                point_2d = obj_panoptic_utils.project_points_on_2D_image(img_idx,
                                                                ...
                                                                image_size,
                                                                filtered_points)
                draw_points(prop_2d_axes, point_2d, 'blue', pt_size=4)
            ...
            if draw_proposals_separate:
                ...
                # Now add the legends
                prop_bev_axes.legend(loc='best', shadow=True, fontsize=35)
                prop_2d_axes.legend(loc='best', shadow=True, fontsize=35)
                prop_3d_axes.legend(loc='upper right', shadow=True, fontsize=35)
                ...
                if draw_projected_2d_boxes:
                    ...
                    if draw_point_cloud:
                        ...
                        point_2d = obj_panoptic_utils.project_points_on_2D_image(img_idx,
                                                                        ...
                                                                        image_size,
                                                                        filtered_points)
                        draw_points(pred_2d_axes, point_2d, 'blue', pt_size=4)
                        pred_2d_axes.legend(loc='best', shadow=True, fontsize=35)
                elif draw_BEV_boxes:
                    pred_fig, pred_bev_axes, pred_3d_axes = \
                        vis_panoptic_utils.visualize_BEV_and_3D(dataset.rgb_image_dir,
                                                                img_idx,
                                                                bev_img,
                                                                display=False,
                                                                fig_size=fig_size_2)

                    draw_predictions_BEV_3D(filtered_gt_objs,
                                            calib_p2,
                                            num_of_predictions,
                                            final_prediction_objs,
                                            final_class_indices,
                    ...
                    if draw_point_cloud:
                        ...
                        point_2d = obj_panoptic_utils.project_points_on_2D_image(img_idx,
                                                                                 ...
                                                                                 image_size,
                                                                                 filtered_points)
                        draw_points(pred_3d_axes, point_2d, 'blue', pt_size=4)
                        # Now add the legends
                        pred_bev_axes.legend(loc='best', shadow=True, fontsize=35)
                        pred_3d_axes.legend(loc='upper right', shadow=True, markerscale=6., fontsize=35)
                else:
                    ...
                    if draw_point_cloud:
                        ...
                        draw_points(pred_3d_axes, point_2d, 'blue', pt_size=4)
                    # Now add the legends
                    pred_3d_axes.legend(loc='upper right', shadow=True, fontsize=35)
                filename = pred_out_dir + '/' + sample_name + '.jpg'
                plt.savefig(filename)
...
def draw_proposals(filtered_gt_objs,
    ...
    for obj in filtered_gt_objs:
        ...
        vis_panoptic_utils.draw_box_bev(
            ...
            color_table=['r', 'y', 'r', 'w'], line_width=5,
            double_line=False,
    ...
    for anchor_idx in range(num_of_proposals):
        ...
        if draw_bev:
            vis_panoptic_utils.draw_box_bev(
                ...
                color_table=['cornflowerblue', 'y', 'r', 'w'], line_width=3,
...
def draw_predictions(filtered_gt_objs,
    ...
    for obj in filtered_gt_objs:
        # Draw groundtruth 2D boxes on Bird's Eye View image
        vis_panoptic_utils.draw_box_bev(
            pred_bev_axes, obj, area_extents, image_shape,
            show_orientation=True,
            color_table=['r', 'y', 'r', 'w'], line_width=5,
            double_line=False)
    ...
    for pred_idx in range(predictions_to_show):
        ...
        vis_panoptic_utils.draw_box_bev(
            pred_bev_axes, pred_obj, area_extents, image_shape,
            show_orientation=True,
...
def draw_predictions_BEV_3D(filtered_gt_objs,
                            p_matrix,
                            predictions_to_show,
                            prediction_objs,
                            prediction_class,
