...
def project_to_image_space(anchors, stereo_calib_p2, image_shape, distortion=None):
    """
    ...
    pts_2d = calib_panoptic_utils.project_to_image(anchor_corners, stereo_calib_p2, dist=distortion)
...
def tf_project_to_image_space(anchors, stereo_calib_p2, image_shape):
    ...
    # pts_2d = tf.Print(pts_2d, ['pts_2d =', pts_2d], summarize=100)

    # Get the min and maxes of image coordinates
    i_axis_points = tf.reshape(pts_2d[0, :], (-1, 8))
    # i_axis_points = tf.Print(i_axis_points, ['i_axis_points =', i_axis_points], summarize=100)
    i_axis_min_points = tf.reduce_min(i_axis_points, axis=1)

    j_axis_points = tf.reshape(pts_2d[1, :], (-1, 8))
    # j_axis_points = tf.Print(j_axis_points, ['j_axis_points =', j_axis_points], summarize=100)
    j_axis_min_points = tf.reduce_min(j_axis_points, axis=1)

    # i_axis_points = tf.reshape(pts_2d[0, :], (-1, 8))
    i_axis_max_points = tf.reduce_max(i_axis_points, axis=1)

    # j_axis_points = tf.reshape(pts_2d[1, :], (-1, 8))
    j_axis_max_points = tf.reduce_max(j_axis_points, axis=1)
    ...
    # box_corners_norm = tf.Print(box_corners_norm, ['box_corners_norm=', box_corners_norm], summarize=100)
...
def project_to_image_tensor(points_3d, cam_intrinsic_matrix, distortion=None):
    """Projects 3D points to 2D points in image space.

    Args:
        points_3d: a list of float32 tensor of shape [3, None]
        cam_intrinsic_matrix: a float32 tensor of shape [3, 3] representing
            the camera intrinsic matrix. (For panoptic dataset)
    ...
    Returns:
        points_2d: a list of float32 tensor of shape [2, None]
            ...
            3D points in image coordinates.
    """
    # cam_intrinsic_matrix = tf.Print(cam_intrinsic_matrix, ['cam_intrinsic_matrix =', cam_intrinsic_matrix], summarize=100)
    # ones_column = tf.ones([1, tf.shape(points_3d)[1]])
    #
    # # Add extra column of ones
    # points_3d_concat = tf.concat([points_3d, ones_column], axis=0)
    ...
    # Multiply camera matrix by the 3D points
    # points_2d = tf.matmul(cam_intrinsic_matrix, points_3d_concat)
    points_2d = tf.matmul(cam_intrinsic_matrix, points_3d)

    # 'Tensor' object does not support item assignment
    # so instead get the result of each division and stack
    # the results
    # points_2d = tf.Print(points_2d, ['points_2d = ', points_2d], summarize=1000)
    points_2d_c1 = points_2d[0, :] / points_2d[2, :]
    points_2d_c2 = points_2d[1, :] / points_2d[2, :]
    stacked_points_2d = tf.stack([points_2d_c1,
                                  points_2d_c2],
...
# def calib_panoptic_utils.project_to_image(point_cloud, p, dist=None):
