...
import pplp.core.format_checker as fc
...
def anchors_to_box_3d(anchors, fix_lw=False):
    ...
    if tensor_format:
        ...
        if fix_lw:
            ...
            box_3d_l = tf.reshape(masked_dimz, [-1])  # Reshape here because sometimes tf.shape(masked_dimz)=[1]
            box_3d_w = tf.reshape(masked_dimx, [-1])
            box_3d_ry = tf.reshape(masked_orientation, [-1])

        box_3d = tf.stack([box_3d_x, box_3d_y, box_3d_z, box_3d_l, box_3d_w, box_3d_h, box_3d_ry], axis=1)
