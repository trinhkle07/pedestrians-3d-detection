...
def random_colors(N, bright=True):
    ...
    # print('hsv = ', hsv)
    # hsv =  [(0.0, 1, 1.0), (0.25, 1, 1.0), (0.5, 1, 1.0), (0.75, 1, 1.0)]
    colors = list(map(lambda c: colorsys.hsv_to_rgb(*c), hsv))
    # print('colors = ', colors)
    # colors = [(1.0, 0.0, 0.0), (1.0, 0.5, 0.0), (0.0, 0.0, 1.0)]
    colors = [(0.5, 0.0, 1.0), (1.0, 0.0, 0.0), (1.0, 0.5, 0.0), (0.5, 1.0, 0.0)]
    # (1.0, 0.0, 0.0) red
    # (1.0, 0.5, 0.0) orange
    # (1.0, 1.0, 0.0) yellow
    # (0.5, 1.0, 0.0) green
...
def save_image_keypoint_and_mask(image, boxes, keypoints, masks, class_ids, class_names,
                                 filename=None, skeleton=[], scores=None, title="",
                                 figsize=(16, 9), ax=None, save_picture=False):
    """
    boxes: [num_instance, (y1, x1, y2, x2, class_id)] in image coordinates.
...
def save_keypoints(image, boxes, keypoints, class_ids, class_names, filename=None,
                   skeleton=[], scores=None, title="",
                   figsize=(16, 9), ax=None, save_picture=False):
    """
    boxes: [num_persons, (y1, x1, y2, x2)] in image coordinates.
...
def draw_rois(image, rois, refined_rois, mask, class_ids, class_names, limit=10):
    ...
    for i, id in enumerate(ids):
        ...
        if class_id:
            ...
            # Mask
            m = maskrcnn_utils.unmold_mask(mask[id], rois[id]
