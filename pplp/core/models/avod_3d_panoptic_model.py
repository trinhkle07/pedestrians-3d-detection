...
from pplp.builders import avod_fc_layers_builder
from pplp.builders import avod_loss_builder
...
class AvodModel(model.DetectionModel):
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        # Create the RpnModel
        self._rpn_avod_panoptic_model = RpnModel(model_config, train_val_test, dataset)
    ...
    def build(self):
        rpn_avod_panoptic_model = self._rpn_avod_panoptic_model

        # Share the same prediction dict as RPN
        prediction_dict = rpn_avod_panoptic_model.build()

        top_anchors = prediction_dict[RpnModel.PRED_TOP_ANCHORS]
        # top_anchors.shape is: (?, 6)
        ground_plane = rpn_avod_panoptic_model.placeholders[RpnModel.PL_GROUND_PLANE]

        class_labels = rpn_avod_panoptic_model.placeholders[RpnModel.PL_LABEL_CLASSES]
        ...
        with tf.variable_scope('avod_projection'):
            ...
            with tf.variable_scope('bev'):
                ...
                bev_proposal_boxes, bev_proposal_boxes_norm = \
                    anchor_panoptic_projector.project_to_bev(
                        avod_projection_in,
                        self.dataset.panoptic_3d_utils.bev_extents)
            ...
            with tf.variable_scope('img'):
                image_shape = tf.cast(tf.shape(
                    rpn_avod_panoptic_model.placeholders[RpnModel.PL_IMG_INPUT])[0:2],
                    tf.float32)
                img_proposal_boxes, img_proposal_boxes_norm = \
                    anchor_panoptic_projector.tf_project_to_image_space(
                        avod_projection_in,
                        rpn_avod_panoptic_model.placeholders[RpnModel.PL_CALIB_P2],
                        image_shape)
        ...
        bev_feature_maps = rpn_avod_panoptic_model.bev_feature_maps
        img_feature_maps = rpn_avod_panoptic_model.img_feature_maps
        ...
        if not (self._path_drop_probabilities[0] ==
            ...
            with tf.variable_scope('avod_path_drop'):
                ...
                img_mask = rpn_avod_panoptic_model.img_path_drop_mask
                bev_mask = rpn_avod_panoptic_model.bev_path_drop_mask
        ...
        anchors_gt = rpn_avod_panoptic_model.placeholders[RpnModel.PL_LABEL_ANCHORS]
        if self._box_rep in ['box_3d', 'box_4ca']:
            boxes_3d_gt = rpn_avod_panoptic_model.placeholders[RpnModel.PL_LABEL_BOXES_3D]
        ...
        with tf.variable_scope('avod_gt_projection'):
            bev_anchor_boxes_gt, _ = anchor_panoptic_projector.project_to_bev(
                anchors_gt, self.dataset.panoptic_3d_utils.bev_extents)
            # Now bev_anchor_boxes_gt is in the BEV, and the top left point is (0,0) point.
            # Camera x is not 0 any more.
            # bev_anchor_boxes_gt = tf.Print(bev_anchor_boxes_gt, ['-=-=-=-=-=-=-line 301(avod) : self.dataset.panoptic_3d_utils.bev_extents =', self.dataset.panoptic_3d_utils.bev_extents], summarize=100)
        ...
        with tf.variable_scope('avod_encode_mb_anchors'):
            ...
            if self._box_rep == 'box_3d':
                ...
                mb_offsets_gt = anchor_panoptic_encoder.tf_anchor_to_offset(
                ...
                mb_orientations_gt = tf.gather(orientations_gt,
                                               mb_gt_indices)
                # mb_orientations_gt = tf.Print(mb_orientations_gt, ['line 358(avod) : mb_orientations_gt =', mb_orientations_gt], summarize=100)
                ...
                if self._box_rep == 'box_8c':
                    mb_boxes_8c_gt = \
                        box_8c_panoptic_encoder.tf_box_3d_to_box_8c(mb_boxes_3d_gt)
                elif self._box_rep == 'box_8co':
                    mb_boxes_8c_gt = \
                        box_8c_panoptic_encoder.tf_box_3d_to_box_8co(mb_boxes_3d_gt)

                # Convert proposals: anchors -> box_3d -> box8c
                proposal_boxes_3d = \
                    box_3d_panoptic_encoder.anchors_to_box_3d(top_anchors, fix_lw=True)
                proposal_boxes_8c = \
                    box_8c_panoptic_encoder.tf_box_3d_to_box_8c(proposal_boxes_3d)

                # Get mini batch offsets
                mb_boxes_8c = tf.boolean_mask(proposal_boxes_8c, mb_mask)
                mb_offsets_gt = box_8c_panoptic_encoder.tf_box_8c_to_offsets(
                ...
                ground_plane = self._rpn_avod_panoptic_model.placeholders[
                    self._rpn_avod_panoptic_model.PL_GROUND_PLANE]
                ...
                # top_anchors = tf.Print(top_anchors, ['line 395(avod) : tf.shape(top_anchors) =', tf.shape(top_anchors)], summarize=100)
                ...
                if self._box_rep == 'box_4ca':
                    # Gather corresponding ground truth orientation for each
                    # mb sample
                    # orientations_gt = tf.Print(orientations_gt, ['---------line 412(avod) : orientations_gt =', orientations_gt], summarize=100)
                    # mb_gt_indices = tf.Print(mb_gt_indices, ['---------line 413(avod) : mb_gt_indices =', mb_gt_indices], summarize=100)
                    mb_orientations_gt = tf.gather(orientations_gt,
                                                   mb_gt_indices)
                    # mb_orientations_gt = tf.Print(mb_orientations_gt, ['---------line 416(avod) : mb_orientations_gt =', mb_orientations_gt], summarize=100)
        ...
        avod_mini_batch_size = \
            self.dataset.panoptic_3d_utils.mini_batch_panoptic_3d_utils.avod_mini_batch_size
        with tf.variable_scope('bev_avod_rois'):
            ...
            bev_input_rois = tf.image.crop_and_resize(
                self._rpn_avod_panoptic_model._bev_preprocessed,
                mb_bev_anchors_norm,
                mb_bev_box_indices,
                (32, 32))
        ...
        with tf.variable_scope('img_avod_rois'):
            ...
            img_input_rois = tf.image.crop_and_resize(
                self._rpn_avod_panoptic_model._img_preprocessed,
                mb_img_anchors_norm,
                mb_img_box_indices,
                (32, 32))
        ...
        with tf.variable_scope('avod_regression'):
            if self._box_rep == 'box_3d':
                prediction_anchors = \
                    anchor_panoptic_encoder.offset_to_anchor(top_anchors,
                                                    all_offsets)
                ...
                prediction_boxes_8c = \
                    box_8c_panoptic_encoder.tf_offsets_to_box_8c(proposal_boxes_8c,
                                                        reshaped_offsets)
                # Convert corners back to box3D
                prediction_boxes_3d = \
                    box_8c_panoptic_encoder.box_8c_to_box_3d(prediction_boxes_8c)
        ...
        with tf.variable_scope('avod_nms'):
            bev_extents = self.dataset.panoptic_3d_utils.bev_extents
    ...
    def sample_mini_batch(self, anchor_box_list_gt, anchor_box_list,
        ...
        with tf.variable_scope('avod_create_mb_mask'):
            ...
            mini_batch_panoptic_3d_utils = self.dataset.panoptic_3d_utils.mini_batch_panoptic_3d_utils
            mb_mask, mb_pos_mask = mini_batch_panoptic_3d_utils.sample_avod_mini_batch(
                max_ious)
            # mb_pos_mask = tf.Print(mb_pos_mask, ['line 683(avod) : tf.shape(mb_pos_mask) =', tf.shape(mb_pos_mask)], summarize=100)
            # mb_pos_mask = tf.Print(mb_pos_mask, ['line 684(avod) : mb_pos_mask =', mb_pos_mask], summarize=100)
            # mb_mask = tf.Print(mb_mask, ['line 685(avod) : tf.shape(mb_mask) =', tf.shape(mb_mask)], summarize=100)
            # mb_mask = tf.Print(mb_mask, ['line 686(avod) : mb_mask =', mb_mask], summarize=100)
            # max_iou_indices = tf.Print(max_iou_indices, ['line 687(avod) : max_iou_indices =', max_iou_indices], summarize=100)
            ...
            mb_class_label_indices = mini_batch_panoptic_3d_utils.mask_class_label_indices(
    ...
    def create_feed_dict(self):
        feed_dict = self._rpn_avod_panoptic_model.create_feed_dict()
        self.sample_info = self._rpn_avod_panoptic_model.sample_info
        return feed_dict

    def loss(self, prediction_dict):
        # Note: The loss should be using mini-batch values only
        loss_dict, rpn_loss = self._rpn_avod_panoptic_model.loss(prediction_dict)
