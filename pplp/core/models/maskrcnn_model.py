...
from pplp.core.maskrcnn import maskrcnn_utils as utils
...
def build_fpn_keypoint_and_feature_graph(rois, feature_maps,
                         image_shape, pool_size, num_keypoints):
    """Builds the computation graph of the keypoint head of Feature Pyramid Network.
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            mrcnn_mask = build_fpn_mask_graph(detection_boxes, mrcnn_feature_maps,
                                              ...
                                              config.MASK_POOL_SIZE,
                                              config.NUM_CLASSES)
            keypoint_mrcnn, keypoint_features = build_fpn_keypoint_and_feature_graph(detection_boxes, mrcnn_feature_maps,
                                                           config.IMAGE_SHAPE,
                                                           config.KEYPOINT_MASK_POOL_SIZE,
                                                           config.NUM_KEYPOINTS)
            ...
            model = KM.Model([input_image, input_image_meta],
                             [detections, mrcnn_class, mrcnn_bbox, rpn_rois, rpn_class, rpn_bbox, mrcnn_mask, keypoint_mcrcnn_prob, keypoint_features],
    ...
    def unmold_keypoint_and_feature_detections(self, detections, mrcnn_keypoints, image_shape, window, mrcnn_mask, features, keypoint_threshold = 0.05):
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.
        ...
        window: [y1, x1, y2, x2] Box in the image where the real image is
                excluding the padding.
        mrcnn_mask: [N, 28, 28, 2]
        features: [N, 56, 56, num_keypoints]

        Returns:
        boxes: [N, (y1, x1, y2, x2)] Bounding boxes in pixels
        class_ids: [N] Integer class IDs for each bounding box
        scores: [N] Float probability scores of the class_id
        full_masks: [height, width, N] Instance masks
        mask: [N, 28, 28, 2]
        features: [N, 56, 56, num_keypoints]
        keypoints:[N, num_keypoints]
        """
        # print('*********** unmold_keypoint_and_feature_detections() ***************')
        # How many detections do we have?
        # Detections array is padded with zeros. Find the first class_id == 0.
        zero_ix = np.where(detections[:, 4] == 0)[0]
        N = zero_ix[0] if zero_ix.shape[0] > 0 else detections.shape[0]

        # Extract boxes, class_ids, scores, class-specific masks, and features
        boxes = detections[:N, :4]
        class_ids = detections[:N, 4].astype(np.int32)
        scores = detections[:N, 5]
        masks = mrcnn_mask[np.arange(N), :, :, class_ids]
        ...
        # print('mrcnn_keypoints.shape = ', mrcnn_keypoints.shape)
        ...
        # print('*********** unmold_keypoint_and_feature_detections() end ***************')
        return boxes, class_ids, scores, keypoints, full_masks, masks, features
    ...
    def detect_keypoint_and_feature_map(self, images, verbose=False):
        """Runs the detection pipeline.
        ...
        scores: [batch, N] float probability scores for the class IDs
        keypoints: [batch, N, num_keypoints, 3] (x, y, v), keypoint x, y coordinate and valid
        features: [batch, N, 56, 56, num_keypoints]
        full_masks: [batch, height, width, N] Instance masks
        mask: [batch, N, 28, 28, 2]
        """
        assert self.mode == "inference", "Create model in inference mode."
        assert len(
            images) == self.config.BATCH_SIZE, "len(images) must be equal to BATCH_SIZE"
        ...
        # [detections, mrcnn_class, mrcnn_bbox, rpn_rois, rpn_class, rpn_bbox, mrcnn_mask, keypoint_mcrcnn_prob]
        detections, mrcnn_class, mrcnn_bbox, \
            rois, rpn_class, rpn_bbox, mrcnn_mask, mrcnn_keypoint_prob, keypoint_features =\
            self.keras_model.predict([molded_images, image_metas], verbose=0)
        features = np.array(keypoint_features)
        if verbose:
            log("rpn_class", rpn_class)
            log("rpn_bbox", rpn_bbox)
        ...
        results = {}
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_keypoints, final_full_masks, final_masks, final_features= \
                self.unmold_keypoint_and_feature_detections(detections[i], mrcnn_keypoint_prob[i], image.shape, windows[i], mrcnn_mask[i], features[i], keypoint_threshold=self.config.KEYPOINT_THRESHOLD)

            if not len(final_rois) == 0:
                if results == {}:
                    results['rois'] = final_rois
                    results['keypoints'] = final_keypoints
                    results['class_ids'] = final_class_ids
                    results['full_masks'] = final_full_masks
                    results['masks'] = final_masks
                    results['scores'] = final_scores
                    results['features'] = final_features
                else:
                    results['rois'] = np.vstack((results['rois'], final_rois))
                    results['keypoints'] = np.vstack((results['keypoints'], final_keypoints))
                    results['class_ids'] = np.vstack((results['class_ids'], final_class_ids))
                    results['full_masks'] = np.vstack((results['full_masks'], final_full_masks))
                    results['masks'] = np.vstack((results['masks'], final_masks))
                    results['scores'] = np.vstack((results['scores'], final_scores))
                    results['features'] = np.vstack((results['features'], final_features))
            else:
                print('No result on this images')
