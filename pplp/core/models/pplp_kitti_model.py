...
from pplp.core import box_8c_encoder
...
from pplp.core.models.rpn_pplp_kitti_model import RpnModel
...
class PplpModel(model.DetectionModel):
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        # Create the RpnModel
        self._rpn_pplp_kitti_model = RpnModel(model_config, train_val_test, dataset)
    ...
    def build(self):
        rpn_pplp_kitti_model = self._rpn_pplp_kitti_model

        # Share the same prediction dict as RPN
        prediction_dict = rpn_pplp_kitti_model.build()

        top_anchors = prediction_dict[RpnModel.PRED_TOP_ANCHORS]
        # top_anchors.shape is: (?, 6)
        ground_plane = rpn_pplp_kitti_model.placeholders[RpnModel.PL_GROUND_PLANE]

        class_labels = rpn_pplp_kitti_model.placeholders[RpnModel.PL_LABEL_CLASSES]
        ...
        with tf.variable_scope('avod_projection'):
            ...
            #         rpn_pplp_kitti_model.placeholders[RpnModel.PL_IMG_INPUT])[0:2],
            #         tf.float32)
            #     img_proposal_boxes, img_proposal_boxes_norm = \
            #         anchor_projector.tf_project_to_image_space(
            #             avod_projection_in,
            #             rpn_pplp_kitti_model.placeholders[RpnModel.PL_CALIB_P2],
            #             image_shape)
        ...
        bev_feature_maps = rpn_pplp_kitti_model.bev_feature_maps
        # img_feature_maps = rpn_pplp_kitti_model.img_feature_maps
        ...
        if not (self._path_drop_probabilities[0] ==
            ...
            with tf.variable_scope('avod_path_drop'):
                ...
                img_mask = rpn_pplp_kitti_model.img_path_drop_mask
                bev_mask = rpn_pplp_kitti_model.bev_path_drop_mask
        ...
        with tf.variable_scope('avod_roi_pooling'):
            ...
            bev_feature_maps = tf.Print(bev_feature_maps, ['line 232(pplp_kitti): bev_feature_maps.shape =', tf.shape(bev_feature_maps)], summarize=1000)
            bev_proposal_boxes_norm_tf_order = tf.Print(bev_proposal_boxes_norm_tf_order, ['line 233(pplp_kitti) : bev_proposal_boxes_norm_tf_order =', bev_proposal_boxes_norm_tf_order], summarize=1000)
            tf_box_indices = tf.Print(tf_box_indices, ['line 234(pplp_kitti) : tf_box_indices =', tf_box_indices], summarize=1000)
            ...
            img_rois = tf.image.resize_images(  # shape=(?, 7, 7, 17)
        ...
        # This may be None
        all_angle_vectors = \
            fc_output_layers.get(pplp_fc_layers_builder.KEY_ANGLE_VECTORS)
        ...
        anchors_gt = rpn_pplp_kitti_model.placeholders[RpnModel.PL_LABEL_ANCHORS]
        if self._box_rep in ['box_3d', 'box_4ca']:
            boxes_3d_gt = rpn_pplp_kitti_model.placeholders[RpnModel.PL_LABEL_BOXES_3D]
        ...
        with tf.variable_scope('avod_apply_mb_mask'):
            # Classification
            mb_mask = tf.Print(mb_mask, ['mb_mask = ', mb_mask], summarize=1000)
            all_cls_logits = tf.Print(all_cls_logits, ['all_cls_logits = ', all_cls_logits], summarize=1000)
        ...
        with tf.variable_scope('avod_encode_mb_anchors'):
            ...
            if self._box_rep == 'box_3d':
                ...
                ground_plane = self._rpn_pplp_kitti_model.placeholders[
                    self._rpn_pplp_kitti_model.PL_GROUND_PLANE]
        ...
        with tf.variable_scope('bev_avod_rois'):
            ...
            bev_input_rois = tf.image.crop_and_resize(
                self._rpn_pplp_kitti_model._bev_preprocessed,
                mb_bev_anchors_norm,
                mb_bev_box_indices,
                (32, 32))
        ...
        #         self._rpn_pplp_kitti_model._img_preprocessed,
        #         mb_img_anchors_norm,
        #         mb_img_box_indices,
        #         (32, 32))
        #
    ...
    def create_feed_dict(self):
        feed_dict = self._rpn_pplp_kitti_model.create_feed_dict()
        self.sample_info = self._rpn_pplp_kitti_model.sample_info
        return feed_dict

    def loss(self, prediction_dict):
        # Note: The loss should be using mini-batch values only
        loss_dict, rpn_loss = self._rpn_pplp_kitti_model.loss(prediction_dict)
        ...
        avod_loss = losses_output[pplp_loss_builder.KEY_AVOD_LOSS]
