...
class RpnModel(model.DetectionModel):
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        self._area_extents = self.dataset.panoptic_3d_utils.area_extents
        self._bev_extents = self.dataset.panoptic_3d_utils.bev_extents
        self._cluster_sizes, _ = self.dataset.get_cluster_info()
        self._anchor_strides = self.dataset.panoptic_3d_utils.anchor_strides
    ...
    def _set_up_input_pls(self):
        ...
        with tf.variable_scope('bev_input'):
            ...
            bev_input_placeholder = self._add_placeholder(tf.float32, bev_dims,
                                                          self.PL_BEV_INPUT) # self.PL_BEV_INPUT is filled with BEV Input Data
            # _bev_input_batches: shape=(batch_size, height, width, channel). batch_size = 1, height x width= 800x700, channel = 2 (1 density, 1 height)
            ...
            self._bev_preprocessed = \
                self._bev_feature_extractor.preprocess_input(
                    self._bev_input_batches, self._bev_pixel_size) # 800x700 pixel size
    ...
    def build(self):
        ...
        img_proposal_input = self.img_bottleneck  # shape=(1, 540, 960, 1)
        ...
        with tf.variable_scope('proposal_roi_pooling'):
            ...
            bev_proposal_rois = tf.image.crop_and_resize(
                bev_proposal_input,  # bev_proposal_input.size =[1 350 400 1]
                self._bev_anchors_norm_pl,  # self._bev_anchors_norm_pl.size =[??? 4]
                tf_box_indices,  # tf_box_indices.size =[???]
                self._proposal_roi_crop_size)
        ...
        with tf.variable_scope('proposals'):
            ...
            # tf.shape(anchors) = [36 6] (example)
            # tf.shape(offsets) = [36 6] (example)
        ...
        with tf.variable_scope('mini_batch'):
            ...
            mini_batch_mask, _ = \
                mini_batch_panoptic_3d_utils.sample_rpn_mini_batch(all_ious_gt)

        # ROI summary images
        rpn_mini_batch_size = \
            self.dataset.panoptic_3d_utils.mini_batch_panoptic_3d_utils.rpn_mini_batch_size
...
# self._img_mrcnn_bbox_input_pl
        ...
        with tf.variable_scope('one_hot_classes'):
            ...
            # Object / Not Object
            min_pos_iou = \
                self.dataset.panoptic_3d_utils.mini_batch_panoptic_3d_utils.rpn_pos_iou_range[0]
    ...
    def create_feed_dict(self, sample_index=None):
        ...
        self._placeholder_inputs[self.PL_BEV_INPUT] = bev_input  # The first few layers are the height map, then flowing with the density map.
    ...
    def _fill_anchor_pl_inputs(self,
        ...
        Args:
            anchors_info: anchor info from mini_batch_panoptic_3d_utils
        ...
        if len(self.dataset.classes) > 1:
            ...
            grid_anchor_boxes_3d = self._anchor_generator.generate(
                ...
                anchor_stride=self._anchor_strides[class_idx],
                ground_plane=ground_plane)
            all_anchor_boxes_3d = grid_anchor_boxes_3d  # For pplp method, we have 448 boxes
        ...
        if not sample_has_labels:
            ...
            voxel_grid_2d = \
                self.dataset.panoptic_3d_utils.create_sliced_voxel_grid_2d(
        ...
        # print('anchor_boxes_3d_to_use.shape = ', anchor_boxes_3d_to_use.shape)
        # print('anchors_ious.shape = ', anchors_ious.shape)
        # print('anchor_offsets.shape = ', anchor_offsets.shape)
        # print('anchor_classes.shape = ', anchor_classes.shape)
        ...
        self._placeholder_inputs[self.PL_ANCHORS] = anchors_to_use  # anchors_to_use
        ...
        # print('^^^^^^^^^^ rpn_panoptic_model.py _fill_anchor_pl_inputs() end^^^^^^^^^^')
