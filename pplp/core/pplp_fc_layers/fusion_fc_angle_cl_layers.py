...
def build(fc_layers_config,
    ...
    Returns:
        ...
        angle_logits: Output angle logits (divide 360 degrees into 16 classes)(or None)
    ...
    if fusion_type == 'early':
        cls_logits, offsets, angle_logits = \
    ...
    return cls_logits, offsets, angle_logits, end_points
...
def build_output_layers(tensor_in,
    ...
    # box_rep =  box_4ca
    # ang_out_size =  2
    # ang_out_size is just to check if the boxes have orietation or not.
    # In this code, we are trying to represent the oritation angle(-pi, pi) with
    # 16 classes. Means that each class symbolize 22.5 degree.
    if ang_out_size > 0:
...
def _early_fusion_fc_layers(num_layers, layer_sizes,
    ...
    with slim.arg_scope(
        ...
        # layer and the out put will contain angle_logits(Nx16).
        output_layers = build_output_layers(fc_drop,
                                            num_final_classes,
                                            box_rep)

    # Out put is : cls_logits, offsets, angle_logits
