...
from pplp.core.pplp_fc_layers import pplp_fc_layer_utils
...
def build(fc_layers_config,
    ...
    Args:
        ...
        img_keypoints: List of pedestrian keyponits in pixels.(transfer to tf.float32 already)
...
def build_output_layers(tensor_in, keypoints_flat,
                        num_final_classes,
                        box_rep):
    """Builds flattened output layers

    Args:
        tensor_in: Input tensor for Classification and Offsets
        keypoints_flat: Input tensor for Angle Unit Vectors
        num_final_classes: Final number of output classes, including
            'Background'
        box_rep: Box representation (e.g. 'box_3d', 'box_8c', 'box_4c')
    ...
    # Offsets
    off_out_size = pplp_fc_layer_utils.OFFSETS_OUTPUT_SIZE[box_rep]
    if off_out_size > 0:
    ...
    # Angle Unit Vectors
    ang_out_size = pplp_fc_layer_utils.ANG_VECS_OUTPUT_SIZE[box_rep]
    # box_rep =  box_4ca
    # ang_out_size =  2
    if ang_out_size > 0:
        ang_out = slim.fully_connected(keypoints_flat,
                                       ang_out_size,
                                       activation_fn=None,
                                       scope='ang_out')
    else:
...
def _early_fusion_fc_layers(num_layers, layer_sizes,
    ...
    # Feature fusion
    fused_features = pplp_fc_layer_utils.feature_fusion(fusion_method,
                                                         input_rois,
                                                         input_weights)
    ...
    keypoints_flat = slim.flatten(img_keypoints)
    ...
    with slim.arg_scope(
        ...
        for layer_idx in range(num_layers):
            ...
            fc_angle_idx = 6 + num_layers + layer_idx

            # Use conv2d instead of fully_connected layers.
            keypoints_flat = slim.fully_connected(keypoints_flat, 34,
                                                  scope='fc{}'.format(fc_angle_idx))
        ...
        # If the box has a angle vector, for example, if box_rep =  'box_4ca',
        # then, ang_out_size = 2. Then fc_drop will be feed into a fully-connected
        # layer and the out put will contain angle_vectors(Nx2).
        output_layers = build_output_layers(fc_drop, keypoints_flat,
                                            num_final_classes,
                                            box_rep)

    # Out put is : cls_logits, offsets, angle_vectors
...
def _deep_fusion_fc_layers(num_layers, layer_sizes,
    ...
    # Apply fusion
    fusion_layer = pplp_fc_layer_utils.feature_fusion(fusion_method,
                                                      input_rois,
                                                      input_weights)
