...
from pplp.core import evaluator_utils
from pplp.core import summary_utils
from pplp.core import trainer_utils

from pplp.core.models.pplp_kitti_model import PplpModel
from pplp.core.models.rpn_model import RpnModel
...
KEY_SUM_PPLP_CLS_LOSS = 'sum_avod_cls_loss'
KEY_SUM_PPLP_REG_LOSS = 'sum_avod_reg_loss'
KEY_SUM_PPLP_TOTAL_LOSS = 'sum_avod_total_loss'
KEY_SUM_PPLP_LOC_LOSS = 'sum_avod_loc_loss'
KEY_SUM_PPLP_ANG_LOSS = 'sum_avod_ang_loss'
KEY_SUM_PPLP_CLS_ACC = 'sum_avod_cls_accuracy'
...
class Evaluator:
    ...
    def _update_avod_box_cls_loc_orient_losses(self,
        ...
        """
        # Get the loss sums from the losses dict
        sum_avod_cls_loss = eval_avod_losses[KEY_SUM_PPLP_CLS_LOSS]
        sum_avod_reg_loss = eval_avod_losses[KEY_SUM_PPLP_REG_LOSS]
        sum_avod_total_loss = eval_avod_losses[KEY_SUM_PPLP_TOTAL_LOSS]
        ...
        avod_orientation_loss = \
            eval_losses[PplpModel.LOSS_FINAL_ORIENTATION]
        ...
        # update the losses sums
        eval_avod_losses.update({KEY_SUM_PPLP_CLS_LOSS:
                                 sum_avod_cls_loss})

        eval_avod_losses.update({KEY_SUM_PPLP_REG_LOSS:
                                 sum_avod_reg_loss})

        eval_avod_losses.update({KEY_SUM_PPLP_TOTAL_LOSS:
                                 sum_avod_total_loss})
        ...
        if (avod_localization_loss > 0.0) and \
            ...
            sum_avod_loc_loss = eval_avod_losses[KEY_SUM_PPLP_LOC_LOSS]
            sum_avod_ang_loss = eval_avod_losses[KEY_SUM_PPLP_ANG_LOSS]

            sum_avod_loc_loss += avod_localization_loss
            sum_avod_ang_loss += avod_orientation_loss

            eval_avod_losses.update({KEY_SUM_PPLP_LOC_LOSS:
                                     sum_avod_loc_loss})
            eval_avod_losses.update({KEY_SUM_PPLP_ANG_LOSS:
                                     sum_avod_ang_loss})
    ...
    def save_prediction_losses_results(self,
        ...
        sum_avod_cls_accuracy = \
            eval_avod_losses[KEY_SUM_PPLP_CLS_ACC]
    ...
    def _create_avod_losses_dict(self):
        ...
        eval_avod_losses[KEY_SUM_PPLP_CLS_LOSS] = 0
        eval_avod_losses[KEY_SUM_PPLP_REG_LOSS] = 0
        eval_avod_losses[KEY_SUM_PPLP_TOTAL_LOSS] = 0

        eval_avod_losses[KEY_SUM_PPLP_LOC_LOSS] = 0
        eval_avod_losses[KEY_SUM_PPLP_ANG_LOSS] = 0

        eval_avod_losses[KEY_SUM_PPLP_CLS_ACC] = 0
    ...
    def get_cls_accuracy(self,
        ...
        if self.full_model:
            ...
            sum_avod_cls_accuracy = eval_avod_losses[KEY_SUM_PPLP_CLS_ACC]
            sum_avod_cls_accuracy += classification_accuracy
            eval_avod_losses.update({KEY_SUM_PPLP_CLS_ACC:
                                     sum_avod_cls_accuracy})
