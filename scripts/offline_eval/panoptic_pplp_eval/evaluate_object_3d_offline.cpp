...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  // std::cout << "####### evaluate_object_3d_offline.cpp #######" << std::endl;
  ...
  for (int64_t i=0; i<indices.size(); i++) {
    ...
    // std::cout << "indices.at(i) = " << indices.at(i) << std::endl;
    ...
    // check for errors
    if (!gt_success) {
      mail->msg("####### ERROR from evaluate_object_3d_offline.cpp #######");
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      ...
      fclose(fp_det);
      cout<< "2D iou threshold = 0.7" << endl;
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_ground[c]) {
      ...
      if(compute_aos_ground)
        fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_orientation_ground.txt").c_str(),"w");
      vector<double> precision[3], aos[3], aos_ground[3];
      ...
      fclose(fp_det);
      cout<< "BEV iou threshold = 0.5" << endl;
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_3d[c]) {
      ...
      fclose(fp_det);
      cout<< "3D iou threshold = 0.5" << endl;
