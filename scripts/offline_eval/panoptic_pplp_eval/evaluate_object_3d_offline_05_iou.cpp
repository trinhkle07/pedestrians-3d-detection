...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
  ...
  for(int32_t i=0; i<gt.size(); i++){
    ...
    // found a valid true positive
      ...
      stat.tp++;
      //   std::cout << "line 560, stat.tp = " << stat.tp << std::endl;
      ...
      if(compute_aos_3d){
        ...
        // cout << "delta_ground = gt[" << i << "].ry - det[" << det_idx << "].ry) = " << gt[i].ry <<"-" << det[det_idx].ry << " = " << (gt[i].ry - det[det_idx].ry) << endl;
      }
  ...
  if(compute_fp) {
    ...
    if (compute_aos_3d) {
        ...
        if(stat.tp>0 || stat.fp>0){
            ...
            //   std::cout << "line 651, stat.similarity_ground = " << stat.similarity_ground << std::endl;
        }
  ...
  for (int32_t i=0; i<thresholds.size(); i++){
    ...
    if(compute_aos){
        aos[i] = pr[i].similarity/(double)(pr[i].tp + pr[i].fp);
          // std::cout << "line 770: aos["<< i << "] = " << "pr["<< i <<"].similarity/(pr["<< i << "].tp + pr["<< i << "].fp) = "<< pr[i].similarity << "/("<< pr[i].tp << "+ "<<pr[i].fp << ") = " << aos[i] << std::endl;
    }


    if(compute_aos_ground){
        aos_ground[i] = pr[i].similarity_ground/(double)(pr[i].tp + pr[i].fp);
        // std::cout << "line 780: aos_ground["<< i << "] = " << "pr["<< i <<"].similarity_ground/(pr["<< i << "].tp + pr["<< i << "].fp) = "<< pr[i].similarity_ground << "/("<< pr[i].tp << "+ "<<pr[i].fp << ") = " << aos_ground[i] << std::endl;
    }
  }
