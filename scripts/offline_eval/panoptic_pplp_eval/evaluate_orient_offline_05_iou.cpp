...
vector<tDetection> loadDetections(string file_name, bool &compute_aos,
  ...
  while (!feof(fp)) {
    ...
    if (fscanf(fp, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
      ...
      // std::cout << "detections.push_back(" << str << ", " << trash << ", " <<
      //   trash << ", " << d.box.alpha << ", " <<  d.box.x1 << ", " <<  d.box.y1 << ", " <<  d.box.x2 << ", " <<  d.box.y2 << ", " <<
      //   d.h << ", " <<  d.w << ", " << d.l << ", " << d.t1 << ", " << d.t2 << ", " << d.t3 << ", " << d.ry << ", " << d.thresh << ")" << std::endl;
...
vector<tGroundtruth> loadGroundtruth(string file_name,bool &success) {
  ...
  while (!feof(fp)) {
    ...
    if (fscanf(fp, "%s %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
      ...
      // std::cout << "groundtruth.push_back(" << str << ", " << g.truncation << ", " <<
      //   g.occlusion << ", " << g.box.alpha << ", " <<  g.box.x1 << ", " <<  g.box.y1 << ", " <<  g.box.x2 << ", " <<  g.box.y2 << ", " <<
      //   g.h << ", " <<  g.w << ", " << g.l << ", " << g.t1 << ", " << g.t2 << ", " << g.t3 << ", " << g.ry << ")" << std::endl;
    }
  }
  fclose(fp);
  success = true;
...
vector<double> getThresholds(vector<double> &v, double n_groundtruth){
  ...
  vector<double> t;
  // std::cout << "line 357, v.size() = "<< v.size() << "; n_groundtruth = " << n_groundtruth << std::endl;

  // v contains the highest detection score of each image
  ...
  for(int32_t i=0; i<v.size(); i++){
    ...
    l_recall = (double)(i+1)/v.size();
    if(i<(v.size()-1))
      // r_recall = (double)(i+2)/n_groundtruth;
      r_recall = (double)(i+2)/v.size();
    else
      r_recall = l_recall;
    // std::cout << "line 376, l_recall = "<< l_recall << "; r_recall = " << r_recall << std::endl;
    ...
    t.push_back(v[i]);
    // std::cout << "line 386, push_back(v["<< i << "]) = " << v[i] << std::endl;
...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
  ...
  // std::cout << "line 479: compute_fp = " << compute_fp << ", compute_aos = " << compute_aos  << ", compute_aos_3d = " << compute_aos_3d << ", thresh = " <<thresh << std::endl;
  if(compute_fp){
      for(int32_t i=0; i<det.size(); i++){
          if(det[i].thresh<thresh){
              ignored_threshold[i] = true;
              // std::cout << "line 484: det[" << i << "].thresh=" << det[i].thresh  << " < thresh=" << thresh << std::endl;
          }
      }
  }
  ...
  for(int32_t i=0; i<gt.size(); i++){
    ...
    //   std::cout << "line 500: for gt: " << (i+1) << "/" << gt.size() << std::endl;
    for(int32_t j=0; j<det.size(); j++){
      ...
      // std::cout << "line 519: overlap = " << overlap << std::endl;
      ...
      if(!compute_fp && overlap>MIN_OVERLAP[metric][current_class] && det[j].thresh>valid_detection){
        ...
        // std::cout << "line 518: det[" << j << "].thresh = " << det[j].thresh << std::endl;
      }
    ...
    if(valid_detection==NO_DETECTION && ignored_gt[i]==0) {
      stat.fn++;
      //   std::cout << "line 545, nothing valid : stat.fn = " << stat.fn << std::endl;
    }
    ...
    // only evaluate valid ground truth <=> detection assignments (considering difficulty level)
        ...
        //   std::cout << "line 551, assigned_detection[" << det_idx << "] = " << det_idx << std::endl;
    }
    ...
    // found a valid true positive
      ...
      stat.tp++;
        // std::cout << "line 563, stat.tp = " << stat.tp << std::endl;
      ...
      if(compute_aos){
        ...
        // cout << "delta_ground = gt[" << i << "].box.alpha - det[" << det_idx << "].box.alpha) = " << gt[i].box.alpha <<"-" << det[det_idx].box.alpha << " = " << (gt[i].box.alpha - det[det_idx].box.alpha) << endl;
      }

      // compute heading difference of detection and ground truth if valid detection orientation was provided
      if(compute_aos_3d){
        delta_ground.push_back(gt[i].ry - det[det_idx].ry);
        // cout << "line 575, delta_ground = gt[" << i << "].ry - det[" << det_idx << "].ry) = " << gt[i].ry <<"-" << det[det_idx].ry << " = " << (gt[i].ry - det[det_idx].ry) << endl;
      }
  ...
  if(compute_fp) {
    ...
    if(compute_aos){
      ...
      if(stat.tp>0 || stat.fp>0){
          ...
          //   std::cout << "line 634, stat.similarity = " << stat.similarity << std::endl;
      }
    ...
    if (compute_aos_3d) {
        ...
        for (int32_t i = 0; i < delta_ground.size(); i++){
            ...
            // std::cout << "line 650, push_back: " << ((1.0 + cos(delta_ground[i])) / 2.0) << std::endl;
        }
        ...
        if(stat.tp>0 || stat.fp>0){
            ...
            // std::cout << "line 656, stat.similarity_ground = " << stat.similarity_ground << std::endl;
        }
...
bool eval_class (FILE *fp_det, FILE *fp_ori,CLASSES current_class,
  ...
  // std::cout << "line 677: eval_class, DIFFICULTY " << difficulty << std::endl;
  ...
  for (int32_t i=0; i<groundtruth.size(); i++){
    ...
    // for (int32_t j=0; j<i_gt.size(); j++){
    //     //   std::cout << "line 674: i_gt["<< j << "] = " << i_gt[j] << std::endl;
    // }
    // for (int32_t j=0; j<i_det.size(); j++){
    //     //   std::cout << "line 677: i_det["<< j << "] = " << i_det[j] << std::endl;
    // }
    ...
    // std::cout << "line 706: computeStatistics(current_class, groundtruth[i], detections[i], dc, i_gt, i_det, false, boxoverlap, metric, false, false);" << std::endl;
    // for (int32_t p_index=0; p_index<groundtruth[i].size(); p_index++){
    //     cout << "computeStatistics groundtruth["<<i<<"]["<<p_index<<"].ry = "<<groundtruth[i][p_index].ry<<endl;
    // }
    // for (int32_t p_index=0; p_index<detections[i].size(); p_index++){
    //     cout << "computeStatistics detections["<<i<<"]["<<p_index<<"].ry = "<<detections[i][p_index].ry<<endl;
    // }
    pr_tmp = computeStatistics(current_class, groundtruth[i], detections[i], dc, i_gt, i_det, false, boxoverlap, metric, false, false);

    // add the highest detection scores of each image to a vector over all images
    for(int32_t j=0; j<pr_tmp.v.size(); j++){
        v.push_back(pr_tmp.v[j]);
        // std::cout <<"For the "<<i<<"th groundtruth, v.push_back(" << (pr_tmp.v[j]) << ")" << std::endl;
    }
  }

  // get scores that must be evaluated for recall discretization
  // cout << "getThresholds, n_gt = " << n_gt << endl;
  // for(int32_t t=0; t<v.size(); t++){
  //     cout << "getThresholds, v["<<t<<"] = " << v[t] << endl;
  // }
  // v holds the available scores for each image
  // n_gt is the number of groundtruth.
  // For example, n_gt=43, v.size()=39
  // Theoretically, thresholds.size() should be equals to N_SAMPLE_PTS. However,
  // when the recall values are not equally distributed, chances are that
  // thresholds.size() < N_SAMPLE_PTS.
  ...
  for (int32_t i=0; i<groundtruth.size(); i++){
    ...
    for(int32_t t=0; t<thresholds.size(); t++){
      tPrData tmp = tPrData();
      //   std::cout << "line 726: computeStatistics(current_class, groundtruth[i], detections[i], dontcare[i], ignored_gt[i], ignored_det[i], true, boxoverlap, metric, metric, compute_aos, compute_aos_ground);" << std::endl;
      // cout << "Second stage!! For the "<<i<<"th image, thresholds[" << t << "] = " << thresholds[t] << endl;
      ...
      //   std::cout << "line 747: pr["<< t << "].similarity = " << pr[t].similarity << std::endl;
      //   std::cout << "line 748: pr["<< t << "].similarity_ground = " << pr[t].similarity_ground << std::endl;
    }
  }
  ...
  for (int32_t i=0; i<thresholds.size(); i++){
    ...
    // std::cout << "line 772: precision["<< i << "] = " << "pr["<< i <<"].tp/(pr["<< i << "].tp + pr["<< i << "].fp) = "<< pr[i].tp << "/("<< pr[i].tp << "+ "<<pr[i].fp << ") = " << precision[i] << std::endl;

    if(compute_aos){
        // Because we only interested in evaluating the orientation for all True
        // Positives, so we don't consider false positives.
        aos[i] = pr[i].similarity/(double)(pr[i].tp);
    }
    ...
    if(compute_aos_ground){
        // Because we only interested in evaluating the orientation for all True
        // Positives, so we don't consider false positives.
        aos_ground[i] = pr[i].similarity_ground/(double)(pr[i].tp);
        // std::cout << "line 796, for recall r["<< i << "] = " << r << ", aos_ground = " << pr[i].similarity_ground << "/" << (double)(pr[i].tp) << " = " << aos_ground[i] << std::endl;
    }
  }
  ...
  for (int32_t i=0; i<thresholds.size(); i++){
    ...
    if(compute_aos_ground){
        ...
        // std::cout << "aos_ground[" << i << "] = " << aos_ground[i] << std::endl;
    }
  ...
  if(compute_aos_ground){
    saveStats(precision, aos_ground, fp_det, fp_ori);
    // std::cout << "line 804 Save aos_ground" << std::endl;
  }
  else{
      saveStats(precision, aos, fp_det, fp_ori);
      // std::cout << "line 808 Save aos" << std::endl;
  }
  return true;
}
...
void printAp(string file_name, vector<double> vals[]){
  ...
  for (int v = 0; v < 3; ++v){
      for (int i = 0; i < vals[v].size(); i++){
          sum[v] += vals[v][i];
          // std::cout << "sum[" << v <<"] += " << "vals["<<v<<"]["<<i<<"] = "<<vals[v][i] << std::endl;
      }
  }

  printf("%s Ap (offline_orient_05_iou): %f %f %f\n", file_name.c_str(), sum[0] / N_SAMPLE_PTS * 100, sum[1] / N_SAMPLE_PTS * 100, sum[2] / N_SAMPLE_PTS * 100);
  //   std::cout << "-- printAp -- :" << file_name.c_str() << " AP: " << sum[0] << ", " << sum[1] << ", " << sum[2] << std::endl;
...
void saveAndPlotPlots(string dir_name,string file_name,string obj_type,vector<double> vals[],bool is_aos){
  ...
  // printf("saveAndPlotPlots to file %s\n", (dir_name + "/" + file_name + ".txt").c_str());
  ...
  for (int v = 0; v < 3; ++v)
      for (int i = 0; i < vals[v].size(); i++)
          sum[v] += vals[v][i];
  printf("%s AP (offline_orient_05_iou): %f %f %f\n", file_name.c_str(), sum[0] / N_SAMPLE_PTS * 100, sum[1] / N_SAMPLE_PTS * 100, sum[2] / N_SAMPLE_PTS * 100);
  //   std::cout << "-- saveAndPlotPlots -- :" << file_name.c_str() << " AP: " << sum[0] << ", " << sum[1] << ", " << sum[2] << std::endl;
...
vector<int64_t> getEvalIndices(const string& result_dir) {
    ...
    if (dir) {
        while (entity = readdir(dir)) {
            ...
            if (len < 10) continue;
            // For example, path = 001100008669.txt
            // len = 16
            // We want to read the last few numbers as the digits.
            int64_t index = atol(path.substr(len - 16, 16).c_str());
            // std::cout << "0.5 iou.cpp, getEvalIndices:: index = " << index << std::endl;
...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  std::vector<int64_t> indices = getEvalIndices(result_dir + "/data/");
  ...
  for (int64_t i=0; i<indices.size(); i++) {
    ...
    // file name
    char file_name[256];
    sprintf(file_name,"%012ld.txt",indices.at(i));
    // std::cout << "file_name = " << file_name << std::endl;
    // std::cout << "eval():: file_name = " << indices.at(i) << ".txt" << std::endl;

    // read ground truth and result poses
    bool gt_success,det_success;
    // std::cout << "Load Groundtruth from " << gt_dir << "/" << file_name << std::endl;
    // std::cout << "Load Detections from " << result_dir << "/data/" << file_name << std::endl;
    vector<tGroundtruth> gt   = loadGroundtruth(gt_dir + "/" + file_name, gt_success);
    // If one of the box.alpha==-10, compute_aos will be set to false.
    ...
    // check for errors
    if (!gt_success) {
      mail->msg("####### ERROR from evaluate_object_3d_offline_05_iou.cpp #######");
      mail->msg("ERROR: Couldn't read: %s of ground truth. Please write me an email!", file_name);
      mail->msg("i = %ld ", i);
      std::cout << "Ground truth not seccess for " << indices.at(i) << std::endl;
      return false;
    }
    if (!det_success) {
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      ...
      fclose(fp_det);
      cout<< "2D iou threshold = 0.5" << endl;
  ...
  // ------------------- evaluate birdview boxes -------------------
  // don't evaluate AOS for birdview boxes and 3D boxes.
  // AOS calculates the differeance of box.alpha; AOS_ground calculates the differeance of ry
  // So we should set compute_aos = false; compute_aos_ground = true;
  compute_aos = false;
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_ground_05_iou.txt").c_str(), "w");
      if(compute_aos_ground)
        fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_orientation_ground_05_iou.txt").c_str(),"w");
      vector<double> precision[3], aos[3], aos_ground[3];
      ...
      fclose(fp_det);
      cout<< "BEV iou threshold = 0.25" << endl;
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_3d[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_3d_05_iou.txt").c_str(), "w");
      if(compute_aos_ground)
        fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_orientation_3d_05_iou.txt").c_str(),"w");
      vector<double> precision[3], aos[3], aos_ground[3];
      ...
      fclose(fp_det);
      cout<< "3D iou threshold = 0.25" << endl;
